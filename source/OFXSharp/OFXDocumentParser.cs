...
namespace OFXSharp
{
    ...
    {
        public OFXDocument Import(FileStream stream, bool ignoreSecurity = false)
        {
            using (var reader = new StreamReader(stream, Encoding.Default))
            {
                return Import(reader.ReadToEnd(), ignoreSecurity);
            }
        }

        public OFXDocument Import(string ofx, bool ignoreSecurity = false)
        {
            return ParseOfxDocument(ofx, ignoreSecurity);
        }

        private OFXDocument ParseOfxDocument(string ofxString, bool ignoreSecurity)
        {
            //If OFX file in SGML format, convert to XML
            if (!IsXmlVersion(ofxString))
            {
                ofxString = SGMLToXML(ofxString, ignoreSecurity);
            }
        ...
        private string SGMLToXML(string file, bool ignoreSecurity)
        {
            var reader = new SgmlReader();

            //Inititialize SGML reader
            reader.InputStream = new StringReader(ParseHeader(file, ignoreSecurity));
            reader.DocType = "OFX";
        ...
        private string ParseHeader(string file, bool ignoreSecurity)
        {
            ...
            CheckHeader(header, ignoreSecurity);
        ...
        private void CheckHeader(string[] header, bool ignoreSecurity)
        {
            ...
            if (header[3] != "SECURITY:NONE" && !ignoreSecurity)
